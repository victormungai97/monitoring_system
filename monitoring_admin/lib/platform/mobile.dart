...
import 'package:monitoring_admin/constants.dart';
import 'package:monitoring_admin/extras.dart';
import 'package:monitoring_admin/title.dart';
  ...
  try {
    if (UserAgent.isMobile()) {
      ...
      if (granted) {
        ...
        if (!isStringEmpty(tag) && tag == 'profile') {
          // Create a FirebaseVisionImage object from your image `File`
          final visionImage = FirebaseVisionImage.fromFile(imageFile);
          // Create an instance of a face detector
          final FaceDetector detector = FirebaseVision.instance.faceDetector(
            FaceDetectorOptions(mode: FaceDetectorMode.accurate),
          );
          // Detect list of images in picture
          final List<Face> faces = await detector.processImage(visionImage);
          // Ensure only 1 image is detected
          if (faces == null || faces.isEmpty || faces.length < 1) {
